...
function extractRoute(text: string): { dep: string | null; arr: string | null } {
  for (const re of [
    ...
    if (m) {
      const dep = m[1], arr = m[2];
      if (ALL_AIRPORTS.has(dep) && ALL_AIRPORTS.has(arr) && dep !== arr) return { dep, arr };
    }
  }
...
function extractDate(text: string): string | null {
  ...
  if ((m = p1.exec(text))) {
    ...
    if (parseInt(y) >= 2020) return `${y}-${mo}-${d}`;
  }
  if ((m = p2.exec(text))) {
  ...
  if ((m = p3.exec(text))) {
    if (parseInt(m[1]) >= 2020) return `${m[1]}-${m[2]}-${m[3]}`;
  }
  if ((m = p4.exec(text))) {
    if (parseInt(m[3]) >= 2020) return `${m[3]}-${m[2]}-${m[1]}`;
  }
  return null;
}
...
// ─── MAIN SCAN — stays under 50 subrequests ──────────────────────────────────
// Budget: 1 list + up to 35 full fetches = max 36 requests
export const scanGmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!googleToken) return { detected: 0, inserted: 0, error: "No Gmail token — please sign out and back in" };

    // 1 request: list up to 35 messages
    const domainQuery = AIRLINE_DOMAINS.map(d => `from:${d}`).join(" OR ");
    const q = `(${domainQuery}) (booking OR confirmation OR itinerary OR "e-ticket" OR "boarding pass") newer_than:1095d`;
    ...
    if (!listRes.ok) {
      ...
      return { detected: 0, inserted: 0, error: `Gmail error ${listRes.status}: ${err.slice(0, 100)}` };
    }
    ...
    // Up to 35 full fetches — stays under 50 total subrequests
    for (const msg of messages) {
      try {
        ...
        if (["survey","newsletter","promotion","offer","miles","feedback","check-in","checkin"]
